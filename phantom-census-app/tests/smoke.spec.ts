...
const PLUGIN_PAGES: Record<string, PluginPage> = {
  ...
  lakebase: {
    navLabel: 'Planner',
    path: '/',
    expectedTexts: ['Phantom Census', 'India Healthcare Desert Map'],
  },
  genie: {
    navLabel: 'Genie',
    path: '/genie',
...
test('smoke test - app loads and displays home page', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Phantom Census' })).toBeVisible();
  await expect(page.getByText('India Healthcare Desert Map')).toBeVisible();
  await expect(page.getByText('token_usage')).toBeVisible();
});
